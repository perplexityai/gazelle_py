...
The plugin walks the directory tree, parses every `.py` for imports via the Rust extractor, and emits stock [`py_library`](https://rules-python.readthedocs.io/en/stable/api/rules_python/python/defs.html#py_library) (one per dir with sources) plus [`py_test`](https://rules-python.readthedocs.io/en/stable/api/rules_python/python/defs.html#py_test) rules (matched against `*_test.py`, `test_*.py`, `tests/**`, `test/**`). Existing canonical package and test targets are refreshed with newly discovered sources while retaining prior test-versus-library classification; sources explicitly owned by sibling targets remain excluded. It also updates `deps` on existing `py_binary` rules from their fully literal `srcs` and optional literal `main`. Computed source expressions, including `glob()` and `select()`, remain unmanaged, and a list containing any computed element is treated as computed in full for dependency inference. Direct literal files in such a list still reserve ownership so Gazelle does not create duplicate targets. Existing rules with computed `deps` are also left unmanaged rather than being rewritten from a partial dependency list. Literal `filegroup` sources claim exact package-local labels only; wildcard-looking strings are not expanded. Use an explicit owner or Gazelle exclusion when generated targets would otherwise overlap computed sources. In file mode, explicitly owned sources do not receive duplicate library or test targets. When one sibling library owns all binary sources, the binary depends on that canonical owner instead of duplicating its dependency list. This keeps package initializers in runfiles and also supports explicit entrypoint libraries. Custom wrappers listed by `python_main_owner_kind` own their local `.py` entrypoint unless they explicitly depend on a same-package library that packages it; Gazelle leaves the wrapper unchanged and assigns the source to that library. Gazelle only maintains wrapper dependencies when their kind is mapped from `py_binary`. `deps` are filled in from a manifest, the first-party `RuleIndex`, or the `pip_parse` repo, in that order.
...
sequenceDiagram
    ...
    Note over Cfg: clone parent pyConfig,<br/>apply directives
...
| `python_main_owner_kind` | _(empty)_ | Treat a custom rule kind's local Python `main` as explicitly owned. Repeat the directive or separate kinds with spaces. Gazelle leaves dependencies unmanaged unless the kind is also mapped from `py_binary`; a same-package library dependency may retain ownership of the entrypoint. |
...
1. `pyproject.toml`, `requirements.txt`, and `requirements.in` (if present) are read once per active `python_root` for declared distribution names.
2. If `python_manifest_file_name` points at a `gazelle_python.yaml`, the file's `modules_mapping` is loaded once per manifest path on first use.
